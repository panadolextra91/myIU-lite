...
- **Definition of Done (pre-push gate) — ALL agents MUST follow:** A phase/task is NOT done until `bash scripts/check.sh` exits 0. That script mirrors the GitHub `ci` job locally (golangci-lint, `go build`, `go vet`, `go test` when `DATABASE_URL` is set, frontend `eslint` + `tsc` + `vite build`) so failures surface BEFORE pushing, not after CI. NEVER claim a phase/wave complete, write a SUMMARY, or push without a green `scripts/check.sh`. A committed `.githooks/pre-push` enforces this on every `git push` once the clone runs the one-time `git config core.hooksPath .githooks`. Requires `golangci-lint` installed at the CI version: `go install github.com/golangci/golangci-lint/cmd/golangci-lint@v1.64.8` (do NOT commit the binary). Emergency bypass only: `git push --no-verify`.
