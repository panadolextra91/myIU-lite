...
- **D-23 — No dedicated CSRF token for the MVP.** Protection relies on browser-native defenses: HttpOnly + SameSite=Lax cookies, strict CORS (exact `FRONTEND_ORIGIN`, `AllowCredentials: true`, wildcard `*` prohibited), and JSON-only state-changing endpoints (reject unsupported content types). The same CORS + cookie rules apply to `POST /auth/refresh`. FE sends `withCredentials: true`; no CSRF token management on either side. Rationale: a single frontend origin + cookie auth, where SameSite=Lax blocks cookies on cross-site POST — a double-submit/synchronizer token would add real BE+FE plumbing for limited benefit at this threat model. **Out of scope:** double-submit tokens, synchronizer token pattern, CSRF token rotation. Design principle: browser-native protections first; add a dedicated CSRF mechanism only when the threat model or deployment architecture requires it. (Resolves research assumption A1.)
