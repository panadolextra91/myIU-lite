...
func TestRequestsIntegration(t *testing.T) {
	...
	// Seed data
	var courseID int64
	err := pool.QueryRow(ctx, `INSERT INTO courses (code, name, term, start_date, end_date) VALUES ('REQ101', 'Req', 'Fall', now(), now() + interval '1 month') RETURNING id`).Scan(&courseID)
	require.NoError(t, err)
	defer pool.Exec(ctx, `DELETE FROM courses WHERE id = $1`, courseID)

	var l1, l2, st int64
	err = pool.QueryRow(ctx, `INSERT INTO users (username, password_hash, role) VALUES ('l1_req', 'hash', 'lecturer') RETURNING id`).Scan(&l1)
	require.NoError(t, err)
	defer pool.Exec(ctx, `DELETE FROM users WHERE id = $1`, l1)

	err = pool.QueryRow(ctx, `INSERT INTO users (username, password_hash, role) VALUES ('l2_req', 'hash', 'lecturer') RETURNING id`).Scan(&l2)
	require.NoError(t, err)
	defer pool.Exec(ctx, `DELETE FROM users WHERE id = $1`, l2)

	err = pool.QueryRow(ctx, `INSERT INTO users (username, password_hash, role) VALUES ('st_req', 'hash', 'student') RETURNING id`).Scan(&st)
	require.NoError(t, err)
	defer pool.Exec(ctx, `DELETE FROM users WHERE id = $1`, st)
